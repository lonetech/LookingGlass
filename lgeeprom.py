...
def rp(dev, addr=0, size=64):
    ...
    r = bytearray(dev.read(1+1+2+64, timeout_ms=1000))
    while r[1:4] != send[:3]:
        r = bytearray(dev.read(1+1+2+64, timeout_ms=1000))
    if len(r) < 1+1+2+64:
        r += bytearray(dev.read(1+1+2+64-len(r), timeout_ms=10))
    #print("read: ", len(r), r)
...
def loadconfig(dev):
    ...
    more=True
    while more:
        more = dev.read(68, blocking=False, timeout_ms=100)
...
for dev in hidapi.enumerate(vendor_id=0x04d8, product_id=0xef7e):
    if dev.product_string == u'HoloPlay':
        ...
        while True:
            ...
            r = hiddev.read(68, blocking=True)
            if len(r)<68:
                r += hiddev.read(68-len(r), blocking=False)
            if r:
                byte = r[0]
